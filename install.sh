...
echo "Next steps:"
echo "  1) Edit .env in $ROOT as needed (GARDENGNOME_DATABASE_URL; GARDENGNOME_DB_APPLY_SCHEMA=0 skips DDL; GARDENGNOME_DB_APPLY_SEEDS=0|auto|1)"
echo "     DB: pending core migrations apply by default; seeds default to auto (prefill when sender_profiles empty). Weather: config/garden.env from template; pip in step 1 unless skipped."
