...
3. **`bash scripts/weather_current.sh`** — first fill of **`weather.weather_*`**; then **`python3 scripts/weather_historical_backfill.py`** optional.
4. User timers: **`bash scripts/setup_cron.sh`** (installed automatically by the installer). For timers to run when logged out, enable **linger**: `loginctl enable-linger "$USER"`.
